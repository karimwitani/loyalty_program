...
stubs returning `501`. There is no test suite yet (`npm test` is a placeholder) — treat any
"Testing" guidance below as the intended pattern, not something already enforced in CI.
...
- No automated tests exist yet; `npm test` in `apps/api/package.json` is a placeholder
  (`echo 'Add test script here'`), as is `npm run lint`. `nodemon.json` already ignores
  `src/**/*.spec.ts`, so the intended pattern is co-located `*.spec.ts` files.
...
fake repository in tests (once tests exist) without changing any service code.
